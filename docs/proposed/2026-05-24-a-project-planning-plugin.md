...
## - [x] Phase 8: Reconciliation mode inside `plan`

**Skills used**: `skill-creator` (re-read for description-tuning conventions and progressive-disclosure structure). Also re-read the existing `plan/SKILL.md`, `plan/jira.md`, `plan/confluence.md`, and `plan/local.md` reference docs to keep voice, vocabulary, and write-discipline patterns consistent.
**Principles applied**: Extended the existing `plan` SKILL.md description with reconciliation-specific trigger phrases ("sync plans", "reconcile with Jira", "what's drifted", "what should graduate") rather than splitting them into a separate skill — the phase spec explicitly forbids a `sync/` directory. Added reconciliation triggers to the **When to trigger** section so the discrimination between default mode and reconciliation mode is visible at the trigger surface, not buried later. Replaced the placeholder **Reconciliation mode** section with a real spec organized around the three load-bearing questions the phase spec names — graduation candidates, drift, stale local items — and required the report to be grouped by those three questions rather than by source, since the user thinks in those terms. Kept the skill itself write-free: it routes acted-on items to the matching reference document's existing write discipline, same as the rest of `plan`. Required per-item confirmations rather than a batch "go ahead" because reconciliation reports tend to span 10+ items across multiple sources, and batching obscures the one or two the user didn't actually want. Added a **Reconciliation** subsection to each of `jira.md`, `confluence.md`, and `local.md` that translates the three questions into the source's specific shape, while explicitly deferring write mechanics to the existing write-discipline sections in the same doc. For `local.md`, added the "sole source of truth = reconciliation is a no-op" short-circuit per the phase spec's "skip" implication, and called out that the manifest's incubation section (`ideas.md`, `decisions.md`) matters more than usual for graduation. For `confluence.md`, required a fresh live read before declaring drift to avoid manufacturing false drift from stale session reads — a hazard specific to Confluence's read-before-write discipline.
